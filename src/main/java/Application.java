...
public class Application {
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException, InterruptedException {
		...
		Flow<BMA280.Command, BMA280.Results, NotUsed> bma280 = I2C.device(bma280config).onBus(1)
				.at(BMA280Constants.DEFAULT_ADDRESS).asFlow(system);

		// Send a SELF-TEST, then a CALIBRATE command to it
		Source.from(Arrays.asList(BMA280.Command.SELFTEST, BMA280.Command.CALIBRATE)).via(bma280).to(Sink.ignore())
				.run(mat);
